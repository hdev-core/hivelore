...
- Votes can be Approve, Reject, Needs Revision, or Mark as Alternate Timeline. Approval percentage counts Approve against **all votes cast** — Approve, Reject, Needs Revision and Alternate Timeline all sit in the denominator. Needs Revision is feedback rather than support, so it does not count toward approval, but it does count against it.
  - _Corrected 2026-08-11._ This previously read "count Approve against Approve + Reject", which contradicted the "at least 5 total internal votes" quorum on the line above: the two halves of the rule counted different populations. Measured against the implementation, 1 Approve + 4 Needs Revision satisfied both conditions (5 total votes, 100% of Approve+Reject) and canonized permanently on-chain — and 5 Approve + 20 Needs Revision did the same. It also meant a voter choosing the more constructive option had less effect on the outcome than one voting Reject. Counting every vote cast in the denominator makes both halves of the rule count the same population.
